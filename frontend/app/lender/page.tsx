        ...
        <div className="max-w-7xl mx-auto px-6 py-4">
          <div className="flex items-center justify-between">
            ...
            <div className="flex items-center gap-4">
              <span className="px-3 py-1 bg-primary-surface-light/30 text-muted text-sm rounded-full border border-primary-surface-light/50">
                🏷️ Lender Mode
              </span>
              
              <Link
                href="/borrower"
                className="px-5 py-2.5 bg-gradient-to-r from-primary-400 to-primary-surface-light hover:from-primary-500 hover:to-primary-700 text-white font-semibold rounded-xl transition shadow-lg hidden sm:block"
              >
                Become Borrower
              </Link>
      ...
      <main className="pt-28 pb-12 px-6">
        <div className="max-w-7xl mx-auto">
          ...
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-8">
            <div>
              <h1 className="text-3xl font-bold text-white mb-2">My Listed Items</h1>
              <p className="text-muted">Manage the items you're renting out</p>
            </div>
            <button
              onClick={() => setShowAddModal(true)}
              className="px-6 py-3 bg-gradient-to-r from-[#9FB798] to-primary-400 hover:from-[#b5cca8] hover:to-[#5ce196] text-white font-semibold rounded-xl transition shadow-lg"
            >
              + Add New Item
            </button>
          </div>
          ...
          {loading ? (
            ...
            <div className="text-center py-20 bg-primary-surface-dark/30 backdrop-blur border border-primary-surface-light/30 rounded-3xl">
              ...
              <h3 className="text-xl font-semibold text-white mb-2">No items listed yet</h3>
              <p className="text-muted mb-6">Start earning by listing your first item!</p>
              <button
                onClick={() => setShowAddModal(true)}
                className="px-6 py-3 bg-gradient-to-r from-primary-400 to-primary-surface-light text-white font-semibold rounded-xl transition"
              >
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {items.map((item) => (
                <div
                  key={item.item_id}
                  className="bg-primary-surface-dark/30 backdrop-blur border border-primary-surface-light/30 rounded-2xl overflow-hidden"
                >
                  <div className="h-40 bg-gradient-to-br from-primary-surface-light/20 to-primary-400/10 flex items-center justify-center">
                  ...
                  <div className="p-5">
                    ...
                    <p className="text-muted/70 text-sm mb-4 line-clamp-2">{item.description}</p>
                    <div className="grid grid-cols-2 gap-2 text-sm">
                      <span className="text-muted">Daily: PKR {Number(item.daily_deposit)}</span>
                      <span className="text-muted">Est.: PKR {Number(item.estimated_price)}</span>
                      <span className="text-muted">Days: {item.min_days}-{item.max_days}</span>
            ...
            <form onSubmit={handleAddItem} className="space-y-4">
              ...
              <div>
                ...
                <textarea
                  ...
                  rows={3}
                  className="w-full px-4 py-3 bg-primary-surface-dark/50 border border-primary-surface-light/50 rounded-xl text-white placeholder-muted/50 focus:outline-none focus:ring-2 focus:ring-primary-400 resize-none"
              ...
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                <div>
                  <label className="block text-sm font-medium text-muted mb-2">Condition</label>
                  <select
                ...
                <div>
                  <label className="block text-sm font-medium text-muted mb-2">Min Days</label>
                  <input
                    type="number"
                    min={1}
                ...
                <div>
                  <label className="block text-sm font-medium text-muted mb-2">Max Days</label>
                  <input
                    type="number"
                    min={1}
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="block text-sm font-medium text-muted mb-2">Daily Deposit (PKR)</label>
                  <input
                    type="number"
                ...
                <div>
                  <label className="block text-sm font-medium text-muted mb-2">Estimated Price (PKR)</label>
                  <input
                    type="number"
              ...
              <div>
                <div className="flex items-center justify-between mb-2">
                  <label className="block text-sm font-medium text-muted">Image URLs (optional)</label>
                  <button
                    type="button"
                    onClick={() => setNewItem({ ...newItem, imageInputs: [...newItem.imageInputs, ""] })}
                    className="text-primary-400 hover:text-muted text-sm"
                  >
                    + Add URL
                  </button>
                </div>
                <div className="space-y-2">
                  {newItem.imageInputs.map((url, idx) => (
                    <div key={idx} className="flex gap-2">
                      <input
                        ...
                        className="flex-1 px-4 py-3 bg-primary-surface-dark/50 border border-primary-surface-light/50 rounded-xl text-white placeholder-muted/50 focus:outline-none focus:ring-2 focus:ring-primary-400"
                        placeholder="https://..."
                      />
                      <button
                        type="button"
                        onClick={() => setNewItem({ ...newItem, imageInputs: newItem.imageInputs.filter((_, i) => i !== idx) })}
                        className="px-3 py-2 bg-primary-surface-dark/50 text-muted rounded-lg border border-primary-surface-light/50 hover:bg-primary-surface-dark"
                      >
                        Remove
                      </button>
                    </div>
                  ...
                  <div className="flex items-center justify-between pt-2">
                    <label className="text-sm text-muted">Or upload from device</label>
                    <div className="flex items-center gap-3">
                      <input
                        ...
                        className="block text-sm text-muted file:mr-3 file:py-2 file:px-3 file:rounded-lg file:border-0 file:bg-primary-surface-light file:text-white hover:file:bg-[#2a8f5e] cursor-pointer"
                      />
                      {uploading && (
              ...
              <div className="flex gap-3 pt-4">
                ...
                <button
                  type="submit"
                  disabled={submitting}
                  className="flex-1 px-4 py-3 bg-gradient-to-r from-primary-400 to-primary-surface-light text-white font-semibold rounded-xl transition disabled:opacity-50"
                >
